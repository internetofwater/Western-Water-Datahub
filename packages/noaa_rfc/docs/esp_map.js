...
function getData() {
  ...
  if (!true) {
    ...
    var src_date =
      "https://www.cbrfc.noaa.gov/wsup/graph/espcond_data.py?fdate=" +
      date_str +
      "&area=CB&qpfdays=0&otype=json" +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    var src_ab_date =
      "https://www.cbrfc.noaa.gov/wsup/graph/espcond_data.py?fdate=" +
      date_str +
      "&area=AB&qpfdays=1&otype=json" +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    var src_wg_date =
      "https://www.cbrfc.noaa.gov/wsup/graph/espcond_data.py?fdate=" +
      date_str +
      "&area=WG&qpfdays=0&otype=json" +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    var src_mb_date =
      "https://www.cbrfc.noaa.gov/wsup/graph/espcond_data.py?fdate=" +
      date_str +
      "&area=MB&qpfdays=1&otype=json" +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    var src_cn_date =
      "https://www.cbrfc.noaa.gov/wsup/graph/west/map/esp_data_cnrfc.py?fdate=" +
      date_str +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    var src_nw_date =
      "https://www.cbrfc.noaa.gov/wsup/graph/west/map/esp_data_nwrfc.py?fdate=" +
      date_str +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    ...
    Promise.all([
      fetch(src_date)
        ...
        .then((res) => {
          cb_date = data2obj(res);
        }),
      fetch(src_ab_date)
        .then((response) => response.json())
        .then((res) => {
          ab_date = data2obj(res);
        }),
      fetch(src_wg_date)
        .then((response) => response.json())
        .then((res) => {
          wg_date = data2obj(res);
        }),
      fetch(src_mb_date)
        .then((response) => response.json())
        .then((res) => {
          mb_date = data2obj(res);
        }),
      fetch(src_cn_date)
        .then((response) => response.json())
        .then((res) => {
          cn_date = data2obj(res);
        }),
      fetch(src_nw_date)
        .then((response) => response.json())
        .then((res) => {
          nw_date = data2obj(res);
        }),
    ]).then((res) => gotData());
    //writestatus();
  } else {
    ...
    var src_mb_latest =
      ...
      (Date.now() / 6) * 60 * 60;
    var src_cn_latest =
      "https://www.cbrfc.noaa.gov/wsup/graph/west/map/esp_data_cnrfc.py?" +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    var src_nw_latest =
      "https://www.cbrfc.noaa.gov/wsup/graph/west/map/esp_data_nwrfc.py?" +
      "&ts=" +
      (Date.now() / 6) * 60 * 60;
    ...
    Promise.all([
      ...
      fetch(src_mb_latest)
        .then((response) => response.json())
        .then((res) => {
          mb_latest = data2obj(res);
        }),
      fetch(src_cn_latest)
        .then((response) => response.json())
        .then((res) => {
          cn_latest = data2obj(res);
        }),
      fetch(src_nw_latest)
        .then((response) => response.json())
        .then((res) => {
          nw_latest = data2obj(res);
        }),
    ]).then((res) => gotData());
  }
}
...
function calcFill(point) {
  ...
  point["link"] =
    "http://www.cbrfc.noaa.gov/wsup/graph/espgraph_hc.html?id=" + point["id"];
  point["plot"] =
    "https://www.cbrfc.noaa.gov/dbdata/wsup/graph/espgraph_hc.py?id=" +
    point["id"];
  point["plotw"] = 700;
  point["ploth"] = 389;
  if (point["wsobasin"] == "CN") {
    point["link"] =
      "https://www.cnrfc.noaa.gov/ensembleProduct.php?prodID=7&id=" +
      point["id"];
    point["plot"] = "";
  }
  if (point["wsobasin"] == "NW") {
    point["link"] =
      "https://www.nwrfc.noaa.gov/water_supply/ws_forecasts.php?id=" +
      point["id"];
    point["plot"] =
