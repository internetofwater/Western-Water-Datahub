...
def create_locations_table(pg_ds: gdal.Dataset) -> None:
    ...
    pg_layer.CreateField(ogr.FieldDefn("ownership", ogr.OFTString))
    ...
    pg_layer.CreateField(ogr.FieldDefn("mng_reg_abbr", ogr.OFTString))
    ...
    # this will be of values "Total" / "Active" / "Live" ; the original CSV has some null values
...
def run_location_load(force_clean_layer=False) -> None:
    ...
    for f in layer:
        ...
        feature.SetField("ownership", row["ownership"])
        ...
        feature.SetField("mng_reg_abbr", row["mng_reg_abbr"])
