...
export interface IPaletteSlice {
  ...
  addPalette: (collectionId: ICollection['id'], palette: PaletteDefinition) => void;
...
> = (set, get) => ({
  ...
  addPalette: (collectionId, palette) =>
    set((state) => {
      const index = state.palettes.findIndex((p) => p.collectionId === collectionId);
  ...
  removePalette: (collectionId) =>
    set((state) => {
      state.palettes = state.palettes.filter((p) => p.collectionId !== collectionId);
    }),
