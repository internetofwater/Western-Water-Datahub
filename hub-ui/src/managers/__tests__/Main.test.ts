...
describe("MainController", () => {
  ...
  test("should create a layer from a valid datasource", () => {
    ...
    expect(layer.name).toBe(
      `${mockDatasource.provider} ${mockDatasource.name} 1`,
    );
  ...
  test("should increment layer name based on existing layers from same datasource", () => {
    ...
    expect(layers[1].name).toBe(
      `${mockDatasource.provider} ${mockDatasource.name} 2`,
    );
  });

  test("should throw an error if datasource is not found", () => {
    expect(() => mainController.createLayer("invalid-id")).toThrow(
