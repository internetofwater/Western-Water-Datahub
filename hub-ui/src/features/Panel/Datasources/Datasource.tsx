...
import { ActionIcon, Box, Button, Group, Stack, Switch, Title } from '@mantine/core';
...
export const Datasource: React.FC<Props> = (props) => {
  ...
  const [checkedGeoFilterRestrictions, setCheckedGeoFilterRestrictions] = useState(false);

  const { isCollectionValid, hasGeoFilterRestrictions } = useValidation(collectionId);
  ...
  useEffect(() => {
    if (
      !checkedGeoFilterRestrictions ||
      [CollectionType.Unknown, CollectionType.EDR, CollectionType.EDRGrid].includes(collectionType)
    ) {
      return;
    }

    void handleUpdate();
  }, [collectionType, checkedGeoFilterRestrictions]);

  useEffect(() => {
    if (hasGeoFilterRestrictions()) {
      setIncludeGeography(true);
    }
    setCheckedGeoFilterRestrictions(true);
  ...
  return (
    <Stack>
      <Group gap="calc(var(--default-spacing) / 2)" justify="space-between" align="center">
        <Title order={3} size="h4" className={styles.datasourceTitle} fs="italic">
          {collection.title}
        </Title>
      ...
      <Group gap="calc(var(--default-spacing) / 2)" justify="space-between" align="flex-start">
        ...
        >
          ...
          {![CollectionType.Unknown, CollectionType.Map].includes(collectionType) && (
            <Switch
              size="xs"
              classNames={{
