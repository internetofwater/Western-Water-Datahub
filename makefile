...
dev: 	
	test -f $(PYGEOAPI_OPENAPI) || OTEL_SDK_DISABLED=false uv run pygeoapi openapi generate $(PYGEOAPI_CONFIG) --output-file $(PYGEOAPI_OPENAPI)
# 	we must be in the pygeoapi-deployment directory for the templates to have the right relative path
	OTEL_SDK_DISABLED=false PYGEOAPI_CONFIG=$(PYGEOAPI_CONFIG) PYGEOAPI_OPENAPI=$(PYGEOAPI_OPENAPI) uv run pygeoapi serve

devNoOTEL:
	OTEL_SDK_DISABLED=true uv run pygeoapi openapi generate $(PYGEOAPI_CONFIG) --output-file $(PYGEOAPI_OPENAPI)
# 	we must be in the pygeoapi-deployment directory for the templates to have the right relative path
	OTEL_SDK_DISABLED=true PYGEOAPI_CONFIG=$(PYGEOAPI_CONFIG) PYGEOAPI_OPENAPI=$(PYGEOAPI_OPENAPI) uv run pygeoapi serve
